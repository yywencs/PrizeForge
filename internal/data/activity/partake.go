...
func (r *Repository) QueryRaffleActivity(ctx context.Context, activityID int64) (*activitybiz.Activity, error) {
	...
	activityKey := infra.GetActivityKey(activityID)
...
func (r *Repository) QueryActivityAccount(ctx context.Context, userID string, activityID int64) (*activitybiz.ActivityAccount, error) {
	...
	key := infra.GetActivityAccountKey(activityID, userID)
...
func (r *Repository) CacheGetOrCreateNoUsedRaffleOrder(ctx context.Context, order *activitybiz.UserRaffleOrder) (*activitybiz.UserRaffleOrder, bool, error) {
	totalKey := infra.GetActivityAccountTotalSurplusKey(order.ActivityID, order.UserID)
	dayKey := infra.GetActivityAccountDaySurplusKey(order.ActivityID, order.UserID, time.Now().Format("2006-01-02"))
	monthKey := infra.GetActivityAccountMonthSurplusKey(order.ActivityID, order.UserID, time.Now().Format("2006-01"))
	pendingOrderKey := infra.GetPendingRaffleOrderKey(order.ActivityID, order.UserID)
...
func (r *Repository) compensatePendingRaffleOrder(ctx context.Context, order *activitybiz.UserRaffleOrder) error {
	totalKey := infra.GetActivityAccountTotalSurplusKey(order.ActivityID, order.UserID)
	dayKey := infra.GetActivityAccountDaySurplusKey(order.ActivityID, order.UserID, order.OrderTime.Format("2006-01-02"))
	monthKey := infra.GetActivityAccountMonthSurplusKey(order.ActivityID, order.UserID, order.OrderTime.Format("2006-01"))
	pendingOrderKey := infra.GetPendingRaffleOrderKey(order.ActivityID, order.UserID)
