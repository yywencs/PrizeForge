...
func (d *Repository) QueryActivityAccountEntity(ctx context.Context, userID string, activityID int64) (*activitybiz.ActivityAccount, error) {
	// 1. 查询总账户
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, activitybiz.ErrRecordNotFound
		}
		return nil, err
	}
	...
	// 4. 组装实体
	activityAccount := &activitybiz.ActivityAccount{
		UserID:            accountPO.UserID,
		ActivityID:        accountPO.ActivityID,
...
func (d *Repository) cacheActivityAccountSnapshot(ctx context.Context, activityAccount *activitybiz.ActivityAccount) error {
	month := time.Now().Format("2006-01")
	day := time.Now().Format("2006-01-02")

	key := infra.GetActivityAccountKey(activityAccount.ActivityID, activityAccount.UserID)
	...
	totalKey := infra.GetActivityAccountTotalSurplusKey(activityAccount.ActivityID, activityAccount.UserID)
	if err := d.redis.Set(&cache.Item{
		Ctx:   ctx,
		Key:   totalKey,
	...
	monthKey := infra.GetActivityAccountMonthSurplusKey(activityAccount.ActivityID, activityAccount.UserID, month)
	if err := d.redis.Set(&cache.Item{
		Ctx:   ctx,
		Key:   monthKey,
	...
	dayKey := infra.GetActivityAccountDaySurplusKey(activityAccount.ActivityID, activityAccount.UserID, day)
	if err := d.redis.Set(&cache.Item{
		Ctx:   ctx,
		Key:   dayKey,
