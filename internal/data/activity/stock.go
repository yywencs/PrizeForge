...
func (d *Repository) ClearActivitySkuStock(ctx context.Context, sku int64) error {
	...
	if err != nil {
		return activitybiz.ErrClearActivitySkuStockError
	}
	return nil
}
...
func (d *Repository) ClearQueueValue(ctx context.Context) error {
	...
	err := d.inspector.DeleteQueue(activitybiz.QueueNameSkuStock, true)
...
func (d *Repository) SubtractionActivitySkuStock(ctx context.Context, skuID int64, activityID int64, userID string, endTime time.Time) (*activitybiz.ActivityResult, error) {
	stockKey := infra.GetActivitySkuStockCountKey(skuID)
	...
	resultKey := infra.GetActivityResultHashKey(activityID, shard)
	...
	pointsResult := fmt.Sprintf("%s_%d", activitybiz.ActivityResultPointsPrefix, 100)
	...
	var activityResult activitybiz.ActivityResult
...
func (d *Repository) ActivitySkuStockConsumeSendQueue(ctx context.Context, skuStockKey *activitybiz.ActivitySkuStockKey) error {
	payload, err := json.Marshal(skuStockKey)
	if err != nil {
		return err
	}

	task := asynq.NewTask(activitybiz.TaskTypeActivitySkuStockConsume, payload)
	info, err := d.queue.Enqueue(task, asynq.Queue(activitybiz.QueueNameSkuStock), asynq.ProcessIn(3*time.Second))
	if err != nil {
		return err
	}
...
func (d *Repository) TakeQueueValue(ctx context.Context, task *asynq.Task) (*activitybiz.ActivitySkuStockKey, error) {
	var skuStockKey activitybiz.ActivitySkuStockKey
	if err := json.Unmarshal(task.Payload(), &skuStockKey); err != nil {
		return nil, fmt.Errorf("json.Unmarshal failed: %w: %w: %w", err, activitybiz.ErrActivitySkuStockKeyUnmarshal, asynq.SkipRetry)
	}
