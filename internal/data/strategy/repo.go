...
import (
	...
	strategybiz "big-market-kratos/internal/biz/strategy"
...
// 4. 构造函数分别返回
func NewStrategyRepository(db *gorm.DB, redis *cache.Cache, queue *asynq.Client, routerDB *infra.DBRouter) strategybiz.Repo {
...
func (sr *strategyRepository) QueryStrategyEntityByStrategyId(ctx context.Context, strategyID int64) (*strategybiz.Strategy, error) {
	var strategyEntity strategybiz.Strategy

	cacheKey := infra.GetStrategyKey(strategyID)
...
func (d *strategyRepository) QueryActivityAccountTotalUseCount(ctx context.Context, userID string, strategyID int64) (int64, error) {
	...
	accountKey := infra.GetActivityAccountKey(activityPO.ActivityID, userID)
	var account activitybiz.ActivityAccount
	...
	err = d.redis.Once(&cache.Item{
		...
		Do: func(*cache.Item) (interface{}, error) {
			...
			return &activitybiz.ActivityAccount{
				UserID:            acc.UserID,
